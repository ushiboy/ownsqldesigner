...
Both are folded into this doc's Design section below rather than filed as a
follow-up, per this project's design-docs rule for refinements found within
...
- `dialectStrategy.ts` — the `DialectStrategy` type consumers call
  (`columnTypes`, `normalizeAutoIncrement(table)`, `isNameTaken`,
  `hasDuplicateNames(names)`, `generateDdl`), importing `Column`/`Table`
  from `../schema/types`. Also exports `buildDialectStrategy(config)`,
  which derives the full `DialectStrategy` from a smaller
  `DialectStrategyConfig` (`columnTypes`, `isAutoIncrementEligible`,
  `isNameTaken`, `generateDdl`) — see "Folding free functions into the
  interface" below for why this split exists.
...
dialect (or strategy) from callers that already hold `schema`:

- `column.ts`/`key.ts`: each mutator already receives `schema`, so it
  resolves `getDialectStrategy(schema.dialect)` once and calls
  `strategy.normalizeAutoIncrement(table)` directly.
- `validation.ts`: `isTableNameAvailable(schema, ...)` — signature
  unchanged, resolves internally. `isColumnNameAvailable(table, name,
dialect, excludeColumnId?)` — gained a `dialect` parameter, since `Table`
  alone doesn't carry it. `describeNameValidity(trimmedName, existingNames,
dialect)` — likewise gained a required `dialect` parameter (see below).
...
Every UI surface that resolves dialect-specific behavior now receives a
`dialect: SqlDialect` prop, sourced from `MainScreenView`'s
`currentSchema?.dialect ?? DEFAULT_SQL_DIALECT` (computed once and reused
for both children below):

- `DialogHost` resolves `getDialectStrategy(dialect)` for
  `strategy.generateDdl(tables)` (replacing the direct `generateSqliteDdl`
  call), and forwards the `dialect` prop itself to `ColumnDialog` and
  `TableNameDialog`.
- `ColumnDialog` resolves `getDialectStrategy(dialect).columnTypes` inside
  the component (replacing a separate `columnTypes` prop from an earlier
  draft — one `dialect` prop is simpler than two related ones) and passes
  `dialect` to `describeNameValidity`.
- `TableNameDialog` passes its new `dialect` prop straight to
  `describeNameValidity`.
- `SidePanel` forwards `dialect` to `TableProperties`, which passes it to
...
domain layer's actual guard was a latent inconsistency, worth the six
components' worth of prop-threading even though it's behaviorally a no-op
while SQLite is the only dialect. `ColumnDialog`'s auto-increment-checkbox
enablement (`fields.type === "INTEGER"`) is the one remaining literal,
dialect-shaped check — it's a UI convenience hint, not the enforcement
point; the domain layer still enforces the real rule via
`strategy.normalizeAutoIncrement` regardless of what the UI allows to be
checked, so it wasn't part of this reversal.
