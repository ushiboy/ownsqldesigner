...
- `pnpm verify:sql` (Docker-based SQL DDL verification, see [0058](../design/0058-local-sql-ddl-verification.md)) is also deliberately **not** part of this sequence: it requires Docker and real database engines, and is not wired into CI. Run it manually when changing DDL generation (`src/domain/dialect/`, `src/domain/sqlite/`, `src/domain/postgresql/`).
