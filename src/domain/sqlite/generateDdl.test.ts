...
describe("generateSqliteDdl", () => {
  ...
  it("generates a plain CREATE TABLE with NOT NULL for non-nullable columns", () => {
    ...
    expect(generateSqliteDdl([users])).toBe(
      "CREATE TABLE users (\n  email TEXT NOT NULL,\n  nickname TEXT\n);",
    );
  });
  ...
  it("includes a declared size in parentheses", () => {
    const users = table({
      id: "t1",
      name: "users",
      columns: [column({ id: "c1", name: "code", type: "TEXT", size: "8" })],
    });

    expect(generateSqliteDdl([users])).toBe("CREATE TABLE users (\n  code TEXT(8)\n);");
  });

  it("renders an autoincrement column inline and omits a table-level PRIMARY KEY", () => {
    const users = table({
      id: "t1",
      name: "users",
      columns: [
  ...
  it("renders a composite PRIMARY KEY with columns in stored order", () => {
    ...
    expect(generateSqliteDdl([memberships])).toBe(
  ...
  it("renders a FOREIGN KEY referencing another table's column", () => {
    ...
    const ddl = generateSqliteDdl([users, posts]);
  ...
  it("deduplicates CREATE INDEX names when two indexes share the same columns", () => {
    ...
    const ddl = generateSqliteDdl([users]);
  ...
  it("joins multiple CREATE TABLE statements with a blank line", () => {
    ...
    expect(generateSqliteDdl([users, posts])).toBe(
      "CREATE TABLE users (\n\n);\n\nCREATE TABLE posts (\n\n);",
    );
  });
});
